...
class Adam(nn.OptimizerBase):
    def __init__(self, lr=0.001, beta_1=0.5, beta_2=0.999, lr_dropout=1.0, lr_cos=0, clipnorm=0.0, name=None, **kwargs):
    ...
    def initialize_variables(self, trainable_weights, vars_on_cpu=True, lr_dropout_on_cpu=False):
        ...
        if e: e.__enter__()
        with tf.variable_scope(self.name):
            ms = {v.name: tf.get_variable(f'ms_{v.name}'.replace(':','_'), v.shape, dtype=v.dtype,
                                          initializer=tf.initializers.constant(0.0), trainable=False) for v in trainable_weights}
    ...
    def get_update_op(self, grads_vars):
        ...
        for g, v in grads_vars:
            ...
            v_t = self.beta_2 * vs + (1.0 - self.beta_2) * tf.square(g)
